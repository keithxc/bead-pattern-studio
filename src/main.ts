...
import { openPatternEditor } from "./editor";
...
const state: ProjectState = {
  ...
  cropMode: "cover", palette: "mard", maxColors: 12, fitMode: "accurate",
...
function template(): string {
  ...
  return `<header>
    ...
    <aside class="panel controls">
      ...
      <section><h2>3 · ${tr("matching")}</h2><label>${tr("palette")}<select id="palette"><option value="mard">${tr("mard")}</option><option value="hama">${tr("hama")}</option><option disabled>Artkal · ${tr("sourceNeeded")}</option><option disabled>Perler · ${tr("sourceNeeded")}</option></select></label><label class="color-limit"><span>${tr("maxColors")} · <b id="maxColorsValue">${state.maxColors ?? tr("unlimited")}</b></span><input id="maxColors" type="range" min="2" max="48" step="1" value="${state.maxColors ?? 12}" ${state.maxColors===null?"disabled":""}></label><label class="unlimited-toggle"><input id="unlimitedColors" type="checkbox" ${state.maxColors===null?"checked":""}> ${tr("unlimited")}</label><div class="segmented"><button data-fit="accurate" class="${state.fitMode==="accurate"?"active":""}">${tr("best")}</button><button data-fit="practical" class="${state.fitMode==="practical"?"active":""}">${tr("practical")}</button></div><small class="hint">${tr("practicalHint")}</small></section>
    ...
    <section class="workspace panel"><div class="workspace-bar"><div class="tabs"><button data-view="original">${tr("original")}</button><button data-view="beads">${tr("preview")}</button><button data-view="pattern">${tr("chart")}</button></div><div class="zoom-controls"><button id="zoomOut" title="${tr("zoomOut")}">−</button><button id="zoomReset" title="${tr("fitView")}"><span id="zoomValue">100%</span></button><button id="zoomIn" title="${tr("zoomIn")}">＋</button></div><span id="status">${state.image?tr("processing"):tr("addImage")}</span></div><div id="stage" class="stage ${state.image?"has-image":""}"><div class="empty"><span>✦</span><strong>${tr("appear")}</strong><small>${tr("tryImage")}</small></div><img id="original" alt="${tr("original")}" src="${imageUrl}"><canvas id="canvas"></canvas></div><div class="summary"><div><small>${tr("grid").toUpperCase()}</small><strong id="gridStat">${state.width} × ${state.height}</strong></div><div><small>${tr("beads").toUpperCase()}</small><strong id="beadStat">—</strong></div><div><small>${tr("colors").toUpperCase()}</small><strong id="colorStat">—</strong></div><div class="actions"><button id="editPattern" disabled>${tr("fineEdit")}</button><button id="exportChart" class="primary" disabled>${tr("exportChart")}</button><button id="exportColor" class="compact" disabled>${tr("exportPreview")}</button></div></div><div id="legend" class="legend"><div class="legend-empty">${tr("usageEmpty")}</div></div></section>
...
function applyZoom(): void {
  ...
  q("#zoomValue").textContent=`${Math.round(zoom*100)}%`;
}
function setZoom(next:number):void {zoom=Math.max(.05,Math.min(8,next));applyZoom()}
function fitZoom():void {
  const canvas=q<HTMLCanvasElement>("#canvas"),stage=q("#stage");
  if(!canvas.width||!canvas.height)return;
  setZoom(Math.min(1,(stage.clientWidth-32)/canvas.width,(stage.clientHeight-32)/canvas.height));
}
...
function paint(): void {
  ...
  const fitCell = Math.max(1, Math.floor(760 / max));
  const cellSize = Math.max(1, Math.min(30, Math.floor(8192/max), state.previewMode === "pattern" ? Math.max(12,fitCell) : fitCell));
  canvas.width = pattern.width * cellSize;
  canvas.height = pattern.height * cellSize;
  canvas.classList.toggle("pattern-canvas",state.previewMode==="pattern");
  ...
  q("#editPattern").removeAttribute("disabled");q("#exportColor").removeAttribute("disabled"); q("#exportChart").removeAttribute("disabled");
...
function bind():void {
  setSelect("#theme",theme);setSelect("#crop",state.cropMode);setSelect("#palette",state.palette);setSelect("#labelMode",state.labelMode);setSelect("#shape",state.beadShape);
  ...
  bindSelect("#crop","cropMode");bindSelect("#palette","palette");bindSelect("#labelMode","labelMode");bindSelect("#shape","beadShape");
  const colorSlider=q<HTMLInputElement>("#maxColors"),unlimited=q<HTMLInputElement>("#unlimitedColors");
  colorSlider.oninput=()=>{state.maxColors=Number(colorSlider.value);q("#maxColorsValue").textContent=colorSlider.value;schedule()};
  unlimited.onchange=()=>{state.maxColors=unlimited.checked?null:Number(colorSlider.value);colorSlider.disabled=unlimited.checked;q("#maxColorsValue").textContent=state.maxColors===null?t(locale,"unlimited"):String(state.maxColors);schedule()};
  ...
  q("#zoomOut").onclick=()=>setZoom(zoom/1.25);q("#zoomIn").onclick=()=>setZoom(zoom*1.25);q("#zoomReset").onclick=fitZoom;
  const stage=q("#stage");let dragging=false,startX=0,startY=0,scrollX=0,scrollY=0;
  stage.onpointerdown=event=>{if(!pattern)return;dragging=true;startX=event.clientX;startY=event.clientY;scrollX=stage.scrollLeft;scrollY=stage.scrollTop;stage.setPointerCapture(event.pointerId);stage.classList.add("dragging")};
  stage.onpointermove=event=>{if(!dragging)return;stage.scrollLeft=scrollX-(event.clientX-startX);stage.scrollTop=scrollY-(event.clientY-startY)};
  stage.onpointerup=stage.onpointercancel=()=>{dragging=false;stage.classList.remove("dragging")};
  stage.onwheel=event=>{if(!pattern)return;event.preventDefault();setZoom(zoom*(event.deltaY<0?1.12:1/1.12))};
  ...
  q("#editPattern").onclick=()=>{if(pattern)openPatternEditor(pattern,{title:t(locale,"fineEdit"),brush:t(locale,"brush"),erase:t(locale,"erase"),pan:t(locale,"pan"),undo:t(locale,"undo"),redo:t(locale,"redo"),fit:t(locale,"fitView"),export:t(locale,"exportChart"),done:t(locale,"done")},()=>exportChartPng(pattern!,"code",t(locale,"shareTitle"),true),paint)};
}
